...
forgotPasswordLink.addEventListener('click', () => {
  ...
  forgotPasswordStep.querySelector('.check-fields').addEventListener('input', () => validateForm(forgotPasswordStep));
})
...
const checkUser = async (email, loaderElement) => {
  ...
  try {
    const response = await fetch(`${baseUrl}/check-account?email=${email}`, {
      method: 'POST',
      headers: {
    ...
    if (form) {
      form.querySelector('.prev-data__email').setAttribute('value', email);
  ...
  fetch(form.action, {
    ...
    .then(data => {
      if (data.status === 'success') {
        ...
        setTimeout(() => {
          window.location.href = data.redirect_url === 'current' ? currentUrl : data.redirect_url;
        }, 1200);
      } else {
        $.toast({
          heading: 'Error!',
...
quantityWrappers.forEach((counter) => {
  const quantityField = counter.querySelector(".quantity-counter__btn--quantity");
